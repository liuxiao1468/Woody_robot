...
def dance1():
	...
	time.sleep(1.5)
	dxl_io.set_goal_position(dict(zip([1, 5, 10], [-90, 0, 90])))
	time.sleep(1.5)
	dxl_io.set_goal_position(dict(zip([1, 5, 10], [0, -90, 0])))
	time.sleep(1.5)
	dxl_io.set_goal_position(dict(zip([1, 5, 10], [-90, 0, 90])))
	time.sleep(1.5)
	dxl_io.set_goal_position(dict(zip([1, 5, 10], [0, -90, 0])))
	time.sleep(1.5)
	dxl_io.set_goal_position(dict(zip([1, 5, 10], [-45, -90, 90])))
	time.sleep(1.5)
...
def music():
	say('Would you like me to play some music?')

def playmusic():
	say('Listen, this is what I like these days')
	os.system('mpg321 rockdemo.mp3') #Credit source in video: https://www.bensound.com/royalty-free-music/rock
	say('It looks like you love my favorite song too!')
...
if __name__=='__main__':
	...
	thread_play = threading.Thread(target = playmusic)
	thread_dance = threading.Thread(target = dance1)
	...
	music()
	time.sleep(2)
	response = 1#yesorno(recognizer, microphone)
	if response == 1:
		thread_play.start()
		thread_dance.start()
		time.sleep(24)
